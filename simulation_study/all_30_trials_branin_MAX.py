...
np.random.seed(0)
sample = np.random.uniform(size=(100,solution[0,0].shape[0]))
...
def max_likelihood_estimate(samples):
    ...
    for no, label in enumerate(sigs):
        ...
        for trial in range(num_trial):
            ...
            for i, s in enumerate(sig_scale):
                ...
                for j, alpha in enumerate(alpha_set):
                    grid_result[i,j,:,trial] = ce.model.obj(sig_inv, alpha)
        ...
        for guess in range(num_ini_guess,NUM_SAMPLES):
            ...
            path = os.path.expanduser('branin_ML/sample_study_MAX_trial30_new/all'+label+'branin_{:d}iter_{:d}init.txt'.format(NUM_SAMPLES, guess))
